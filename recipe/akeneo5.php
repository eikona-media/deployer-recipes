...
task('deploy:yarn_install', function () {
    // Run with custom Node.js Path for Node.js 12
...
task('deploy:yarn_compile', function () {
    // Run with custom Node.js Path for Node.js 12
    run('cd {{release_path}} && PATH="{{deploy_nodejs_path}}:$PATH" {{bin/yarn}} --cwd=vendor/akeneo/pim-community-dev/akeneo-design-system install --frozen-lockfile');
    run('cd {{release_path}} && PATH="{{deploy_nodejs_path}}:$PATH" {{bin/yarn}} --cwd=vendor/akeneo/pim-community-dev/akeneo-design-system run lib:build');
